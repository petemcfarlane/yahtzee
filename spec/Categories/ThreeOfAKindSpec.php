...
class ThreeOfAKindSpec extends ObjectBehavior
{
    ...
    function it_evaluates_true_if_there_are_three_of_the_same_value(DiceRoll $diceRoll)
    {
        $diceRoll->values()->willReturn([1, 2, 4, 2, 2]);
        $this->evaluate($diceRoll)->shouldBe(true);
    }

    function it_evaluates_false_otherwise(DiceRoll $diceRoll)
    {
        $diceRoll->values()->willReturn([1, 2, 3, 4, 4]);
    ...
    {
        $diceRoll->values()->willReturn([1, 2, 4, 2, 2]);
        $this->score($diceRoll)->shouldBe(11);
    }
